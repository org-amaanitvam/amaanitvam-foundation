      ...
      <div className="w-full max-w-220 h-auto md:h-130 bg-transparent rounded-2xl flex flex-col md:flex-row overflow-hidden relative z-10 shadow-2xl">
        ...
        <div className="flex-1 flex flex-col justify-center p-8 md:p-10 bg-white rounded-b-2xl md:rounded-bl-none md:rounded-r-2xl">
          <div className="w-full flex flex-col items-center">
            <div className="flex justify-center gap-6 mb-6 w-full max-w-[320px] border-b border-gray-200 pb-3">
              <button
                ...
                className={`text-sm font-bold uppercase tracking-widest pb-1 cursor-pointer transition-all duration-200 disabled:opacity-40 ${
                  !showReset && !show2FA ? 'text-[#6b1d44] border-b-2 border-[#6b1d44] font-black' : 'text-gray-400 hover:text-gray-700'
                }`}
              >
                Sign In
              </button>
            ...
            {error && <div className="w-full max-w-[320px] bg-red-50 text-red-700 text-xs p-2.5 rounded-lg border border-red-200 mb-4 text-center">{error}</div>}
            {resetSuccess && <div className="w-full max-w-[320px] bg-green-50 text-green-700 text-xs p-2.5 rounded-lg border border-green-200 mb-4 text-center">{resetSuccess}</div>}

            {show2FA ? (
              <form onSubmit={handleVerify2FA} className="w-full max-w-[320px] flex flex-col items-center">
                <p className="text-xs text-gray-500 text-center mb-4 leading-relaxed">Enter verification code.<br/><span className="text-[10px] text-gray-400">Demo code: 123456</span></p>
                <input type="text" required value={code2fa} onChange={(e) => setCode2fa(e.target.value)} className="w-full border-b border-gray-300 py-2 text-center text-xl tracking-[0.3em] font-bold text-gray-800 outline-none focus:border-[#6b1d44] transition-colors bg-transparent" maxLength={6}/>
                <div className="w-full flex justify-between items-center mt-2">
                  <button type="button" onClick={() => {setShow2FA(false); setTempCredentials(null); setCode2fa(''); setError('');}} className="text-xs text-gray-400 hover:text-gray-600 bg-transparent cursor-pointer flex items-center gap-1"><ArrowLeft className="w-3 h-3" /> Back</button>
                  <button type="submit" disabled={isLoading} className="bg-[#6b1d44] text-white text-xs font-bold uppercase tracking-wider py-2.5 px-6 rounded-full shadow-md hover:opacity-90 disabled:opacity-50 cursor-pointer">{isLoading ? 'Verifying...' : 'Verify'}</button>
                </div>
              </form>
            ) : !showReset ? (
              <form onSubmit={handleLogin} className="w-full max-w-[320px] flex flex-col items-center">
                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@amaanitvam.org" required className="w-full border-b border-gray-300 py-2 text-sm text-gray-800 outline-none focus:border-[#6b1d44] mb-5 bg-transparent" />
                <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" required className="w-full border-b border-gray-300 py-2 text-sm text-gray-800 outline-none focus:border-[#6b1d44] mb-6 bg-transparent" />
                <div className="w-full flex justify-between items-center mt-2">
                  <button type="button" onClick={() => {setShowReset(true); setError('');}} className="text-xs text-gray-400 hover:text-[#6b1d44] bg-transparent cursor-pointer">Forgot Password?</button>
                  <button type="submit" disabled={isLoading} className="bg-[#6b1d44] text-white text-xs font-bold uppercase tracking-wider py-2.5 px-6 rounded-full shadow-md hover:opacity-90 disabled:opacity-50 cursor-pointer">{isLoading ? 'Signing in…' : 'Sign In'}</button>
                </div>
              </form>
            ) : (
              <form onSubmit={handleResetPassword} className="w-full max-w-[320px] flex flex-col items-center">
                <input id="reset-email" type="email" value={resetEmail} onChange={(e) => setResetEmail(e.target.value)} placeholder="you@amaanitvam.org" required className="w-full border-b border-gray-300 py-2 text-sm text-gray-800 outline-none focus:border-[#6b1d44] mb-6 bg-transparent" />
                <div className="w-full flex justify-between items-center mt-2">
                  <button type="button" onClick={() => {setShowReset(false); setError(''); setResetSuccess('');}} className="text-xs text-gray-400 hover:text-gray-600 bg-transparent cursor-pointer">Back to Sign In</button>
                  <button type="submit" className="bg-[#6b1d44] text-white text-xs font-bold uppercase tracking-wider py-2.5 px-6 rounded-full shadow-md hover:opacity-90 cursor-pointer">Send Reset Link</button>
                </div>
              </form>
            )}
          </div>
